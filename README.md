...
### Khởi chạy Redis + PostGIS (Docker, tùy chọn)
```bash
...
### Bật PostGIS trên PostgreSQL (một lần)
```bash
psql -U postgres -d flood_rescue_db -f sql/postgis_enable.sql
```

### Tính năng nâng cao (README/DESIGN)
| Tính năng | Mô tả |
|-----------|--------|
| **WebSocket** | Thông báo real-time qua STOMP `/ws` → `/topic/notifications/{userId}` |
| **Redis Geo** | Gợi ý đội gần nhất — UI gọi `GET /api/rescue-requests/{id}/nearby-teams` |
| **PostGIS** | Fallback truy vấn `ST_Distance` khi Redis không có kết quả |
| **Rate limit** | Redis: `app.rate-limit.requests-per-minute` (mặc định 120/phút) |
...
### Kiểm thử
Xem **[TEST_CHECKLIST.md](./TEST_CHECKLIST.md)** — checklist môi trường, kịch bản E2E theo từng role, và test kỹ thuật (WebSocket, Redis Geo, PostGIS, OSRM).
